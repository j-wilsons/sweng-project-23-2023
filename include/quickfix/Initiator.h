...
public:
  Initiator( Application&, MessageStoreFactory&,
             const SessionSettings& ) QUICKFIX_THROW( ConfigError );
  Initiator( Application&, MessageStoreFactory&,
             const SessionSettings&, LogFactory& ) QUICKFIX_THROW( ConfigError );

  virtual ~Initiator();

  /// Start initiator.
  void start() QUICKFIX_THROW( ConfigError, RuntimeError );
  /// Block on the initiator
  void block() QUICKFIX_THROW( ConfigError, RuntimeError );
  /// Poll the initiator
  bool poll( double timeout = 0.0 ) QUICKFIX_THROW( ConfigError, RuntimeError );
...
private:
  void initialize() QUICKFIX_THROW( ConfigError );

  /// Implemented to configure acceptor
  virtual void onConfigure( const SessionSettings& ) QUICKFIX_THROW( ConfigError ) {};
  /// Implemented to initialize initiator
  virtual void onInitialize( const SessionSettings& ) QUICKFIX_THROW( RuntimeError ) {};
